...
void TextureResourceManager::init(VkDevice device, VkPhysicalDevice physicalDevice, VkQueue graphicsQueue,
...
VkImageView TextureResourceManager::loadHDRTexture(const std::string& hdrPath)
{
    ...
    vulkanUtils.createBuffer(device, physicalDevice, imageSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
    ...
    vulkanUtils.createImage(device, physicalDevice, texWidth, texHeight, VK_FORMAT_R32G32B32A32_SFLOAT,
                            VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
                            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, sourceHDRImage, sourceHDRImageMemory);
    VkCommandPool commandPool = commandManager->getCommandPool();
    vulkanUtils.transitionImageLayout(device, commandPool, graphicsQueue, sourceHDRImage, VK_FORMAT_R32G32B32A32_SFLOAT,
                                      VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
    vulkanUtils.copyBufferToImage(device, commandPool, graphicsQueue, stagingBuffer, sourceHDRImage,
                                  static_cast<uint32_t>(texWidth), static_cast<uint32_t>(texHeight));
    vulkanUtils.transitionImageLayout(device, commandPool, graphicsQueue, sourceHDRImage, VK_FORMAT_R32G32B32A32_SFLOAT,
                                      VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);

    sourceHDRImageView =
        vulkanUtils.createImageView(device, sourceHDRImage, VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_ASPECT_COLOR_BIT);
...
void TextureResourceManager::createEnvironmentMap()
{
    ...
    vulkanUtils.createImage(device, physicalDevice, environmentMapSize, environmentMapSize,
                            VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_TILING_OPTIMAL,
                            VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
                            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, environmentMapImage, environmentMapImageMemory,
                            VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT, 6);

    environmentMapImageView = vulkanUtils.createImageView(device, environmentMapImage, VK_FORMAT_R32G32B32A32_SFLOAT,
    ...
    for (uint32_t i = 0; i < 6; i++)
    {
        environmentMapFaceImageViews[i] = vulkanUtils.createImageView(
            device, environmentMapImage, VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_ASPECT_COLOR_BIT,
...
void TextureResourceManager::createIrradianceMap()
{
    ...
    vulkanUtils.createImage(device, physicalDevice, irradianceMapSize, irradianceMapSize, VK_FORMAT_R32G32B32A32_SFLOAT,
                            VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
                            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, irradianceMapImage, irradianceMapImageMemory,
                            VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT, 6);

    irradianceMapImageView = vulkanUtils.createImageView(device, irradianceMapImage, VK_FORMAT_R32G32B32A32_SFLOAT,
                                                         VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_VIEW_TYPE_CUBE,
                                                         6 // 立方体贴图的层数
    );
    for (uint32_t i = 0; i < 6; i++)
    {
        irradianceMapFaceImageViews[i] = vulkanUtils.createImageView(
            device, irradianceMapImage, VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_ASPECT_COLOR_BIT,
            VK_IMAGE_VIEW_TYPE_2D, // 每个面是一个 2D 图像
            1,                     // 每个 ImageView 只访问一个层
            i                      // 指定访问的层索引（对应立方体贴图的面）
        );
...
void TextureResourceManager::createPrefilteredMap()
{
    ...
    vulkanUtils.createImage(device, physicalDevice, prefilteredMapSize, prefilteredMapSize,
                            VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_TILING_OPTIMAL,
                            VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT |
                                VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT,
                            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, prefilteredMapImage, prefilteredMapImageMemory,
                            VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT,
                            6,        // 立方体贴图的层数
                            mipLevels // 设置 Mipmap 级别
    );

    prefilteredMapImageView = vulkanUtils.createImageView(device, prefilteredMapImage, VK_FORMAT_R32G32B32A32_SFLOAT,
                                                          VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_VIEW_TYPE_CUBE,
                                                          6, // 立方体贴图的层数
                                                          0,
                                                          mipLevels, // 设置 Mipmap 级别
    ...
    for (uint32_t mipLevel = 0; mipLevel < mipLevels; ++mipLevel)
    {
        for (uint32_t face = 0; face < 6; ++face)
        {
            prefilteredMapFaceImageViews[mipLevel][face] = vulkanUtils.createImageView(
                device, prefilteredMapImage, VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_ASPECT_COLOR_BIT,
                VK_IMAGE_VIEW_TYPE_2D, // 每个面是一个 2D 图像
                1,                     // 每个 ImageView 只访问一个层
...
void TextureResourceManager::createBRDFLUT()
{
    ...
    vulkanUtils.createImage(device, physicalDevice, brdfLUTMapSize, brdfLUTMapSize, VK_FORMAT_R32G32B32A32_SFLOAT,
                            VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
                            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, brdfLUTImage, brdfLUTImageMemory);

    brdfLUTImageView =
        vulkanUtils.createImageView(device, brdfLUTImage, VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_ASPECT_COLOR_BIT);
}
