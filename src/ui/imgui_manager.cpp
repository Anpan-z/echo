                        ...
                        VertexResourceManager& vertexResourceManager, CommandManager& commandManager)
{
    ...
    initInfo.MinImageCount = vulkanContext.querySwapChainSupport(vulkanContext.getPhysicalDevice())
                                 .capabilities.minImageCount; // 最小图像数量
    initInfo.ImageCount =
        vulkanContext.querySwapChainSupport(vulkanContext.getPhysicalDevice()).capabilities.minImageCount + 1;
...
void ImGuiManager::recreatWindow()
{
    textureCache.clear();
    textures.clear();
    // ImGui::SetNextWindowSize(ImVec2((float)swapChainManager->getSwapChainExtent().width,
    // (float)swapChainManager->getSwapChainExtent().height));
}
...
void ImGuiManager::addTexture(const VkImageView* imageView, VkSampler sampler, VkImageLayout imageLayout)
{
    ...
    // offScreenTextureId = reinterpret_cast<ImTextureID>(ImGui_ImplVulkan_AddTexture(sampler, *imageView,
    // imageLayout));
    ImTextureID textureID =
        reinterpret_cast<ImTextureID>(ImGui_ImplVulkan_AddTexture(sampler, *imageView, imageLayout));
...
VkExtent2D ImGuiManager::renderImGuiInterface()
{
    ...
    ImGui::SetNextWindowSize(
        ImVec2((float)swapChainManager->getSwapChainExtent().width - 300,
               (float)swapChainManager->getSwapChainExtent().height - menuBarHeight)); // 窗口大小减去菜单栏高度
    ...
    // 计算纹理的宽高比
    // ImVec2 textureSize = ImVec2((float)swapChainManager->getSwapChainExtent().width,
    // (float)swapChainManager->getSwapChainExtent().height); // 纹理的原始尺寸 float aspectRatio = textureSize.x /
    // textureSize.y; if (availableSize.x / availableSize.y > aspectRatio) {
