...
void RenderTarget::init(VkDevice device, VkPhysicalDevice physicalDevice, SwapChainManager& swapChainManager, VkRenderPass offScreenRenderPass, VkRenderPass renderPass) {
    this->device = device;
...
void RenderTarget::createDepthResources(VkExtent2D imageExtent) {
    ...
    for(size_t i = 0; i < depthImages.size(); i++) {
        ...
        vulkanUtils.createImage(device, physicalDevice, imageExtent.width, imageExtent.height, depthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depthImages[i], depthImageMemories[i]);
        depthImageViews[i] = vulkanUtils.createImageView(device, depthImages[i], depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT);
...
void RenderTarget::createOffScreenResources(VkExtent2D imageExtent) {
    ...
    for (size_t i = 0 ; i < offScreenImages.size(); i++) {
        ...
        vulkanUtils.createImage(device, physicalDevice, imageExtent.width, imageExtent.height, offScreenFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, offScreenImages[i], offScreenImageMemories[i]);
        offScreenImageViews[i] = vulkanUtils.createImageView(device, offScreenImages[i], offScreenFormat, VK_IMAGE_ASPECT_COLOR_BIT);
    }
}
