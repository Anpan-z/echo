...
// 获取单例实例
VulkanUtils& VulkanUtils::getInstance()
{
    static VulkanUtils instance; // 静态局部变量，确保全局唯一
    return instance;
}
...
VkImageView VulkanUtils::createImageView(VkDevice device, VkImage image, VkFormat format,
                                         VkImageAspectFlags aspectFlags, VkImageViewType viewType, uint32_t layerCount,
                                        ...
                                        VkImageLayout newLayout)
{
    ...
    {
        // 从 UNDEFINED 转换到 GENERAL
        barrier.srcAccessMask = 0;                         // 初始状态没有访问
        barrier.dstAccessMask = VK_ACCESS_SHADER_READ_BIT; // 目标是着色器写入
    ...
    else if (oldLayout == VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL && newLayout == VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL)
    {
        ...
        sourceStage = VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT |
                      VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT; // Stage where shader read happened
    ...
    else if (oldLayout == VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL && newLayout == VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL)
    {
        ...
        destinationStage = VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT |
                           VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT; // Stage where shader read will happen
    }
    else
    {
...
VkFormat VulkanUtils::findSupportedFormat(VkPhysicalDevice physicalDevice, const std::vector<VkFormat>& candidates,
...
void VulkanUtils::endSingleTimeCommands(VkDevice device, VkCommandPool commandPool, VkQueue graphicsQueue,
                                        VkCommandBuffer commandBuffer)
{
...
void VulkanUtils::copyBuffer(VkDevice device, VkCommandPool commandPool, VkQueue graphicsQueue, VkBuffer srcBuffer,
...
void VulkanUtils::copyBufferToImage(VkDevice device, VkCommandPool commandPool, VkQueue graphicsQueue, VkBuffer buffer,
...
std::string VulkanUtils::readFileToString(const std::string& filename)
{
...
VkShaderModule VulkanUtils::createShaderModule(VkDevice device, VkShaderModuleCreateInfo& createInfo)
{
